...
//!
//! This module owns the API-key auth-step state machine, renders its UI, and
//! handles auth-scoped keyboard input.
...
impl AuthModeWidget {
    ...
    fn render_pick_mode(&self, area: Rect, buf: &mut Buffer) {
        let mut lines: Vec<Line> = vec![
            "  Connect an API key for usage-based billing".into(),
            "".into(),
        ];
